...
from recognizer import main_rec
...
class MainUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with open("nameslist.txt", "r") as f:
            ...
            for i in z:
                names.add(i)
        self.title_font = tkfont.Font(family='Helvetica', size=16, weight="bold")#设置字体
        self.title("Online Video Evaluation System")#窗体名称
        self.resizable(False, False)#????
        self.geometry("500x250")#窗口大小
        self.protocol("WM_DELETE_WINDOW", self.on_closing)
        self.active_name = None
        container = tk.Frame(self)#另一个界面？？
        container.grid(sticky="nsew")#grid 管理器
...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            self.controller = controller#继承主窗口的风格
            #load = Image.open("homepagepic.png")
            #load = load.resize((250, 250), Image.ANTIALIAS)
            render = PhotoImage(file='homepagepic.png')#加载图片
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.user_name = tk.Entry(self, borderwidth=3, bg="lightgrey", font='Helvetica 11')#文本框获取姓名
    ...
    def start_training(self):
        global names
        if self.user_name.get() == "None":#获取姓名
            messagebox.showerror("Error", "Name cannot be 'None'")
            return
        elif self.user_name.get() in names:#已有姓名
...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.menuvar = tk.StringVar(self)#获取改动的值
...
class PageThree(tk.Frame):
    ...
    def capimg(self):
        ...
        x = start_capture(self.controller.active_name)#人脸捕捉获取人脸的数据集
        self.controller.num_of_images = x#帧数
        self.numimglabel.config(text=str("Number of images captured = "+str(x)))

    def trainmodel(self):
        if self.controller.num_of_images < 10:
            messagebox.showerror("ERROR", "No enough Data, Capture at least 10 images!")
            return
        train_classifer(self.controller.active_name)#形成人脸训练后的LBP
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Sign in", font='Helvetica 16 bold')
        label.grid(row=0,column=0, sticky="ew")
        button1 = tk.Button(self, text="Sign in", command=self.openwebcam, fg="#ffffff", bg="#263942")
    ...
    def openwebcam(self):
        score=main_app(self.controller.active_name)
        if(score==False):
            messagebox.showerror("ERROR", "No such users")
            self.controller.show_frame("StartPage")

        else:
            messagebox.showerror("succeed","succeed")
...
class PageFive(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Online evaluation", font='Helvetica 16 bold')
        label.grid(row=0,column=0, sticky="ew")
        button1 = tk.Button(self, text="Online evaluation", command=self.openwebcam, fg="#ffffff", bg="#263942")
        #button2 = tk.Button(self, text="Emotion Detection", command=self.emot, fg="#ffffff", bg="#263942")
        #button3 = tk.Button(self, text="Gender and Age Prediction", command=self.gender_age_pred, fg="#ffffff", bg="#263942")
        button4 = tk.Button(self, text="Go to Home Page", command=lambda: self.controller.show_frame("StartPage"), bg="#ffffff", fg="#263942")
        button1.grid(row=1,column=0, sticky="ew", ipadx=5, ipady=4, padx=10, pady=10)
    ...
    def openwebcam(self):
        main_rec(self.controller.active_name)
app = MainUI()
